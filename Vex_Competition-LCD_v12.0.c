...
#pragma config(Sensor, in7,    forkPot,        sensorNone)
...
task raiseOrLowerFork()
{
	...
	const int kMinPosf = 250;
	const int kMaxPosf = 4000;
	float error,input;
...
//	float ITerm;
	const float outMax=128,outMin=-128;
	int nPositionToHold;
	float kdf=0.040,kpf=0.048;
	bool bLastUpdateWasButtonPress = false;
	bool bButtonsHaveBeenPressed   = false;
	while(true)
	{
		if (vexRT[Btn6D]==1)// button is puchsed indicating ARM should be lowered
		{
			if (SensorValue[forkPot] < kMinPosf)
			{
				motor[fork] = 0; // Arm is fully lowered. Stop the motor
			}
		...
		else if (vexRT[Btn6U]==1)// button is puchsed indicating ARM should be raised
		{
			if (SensorValue[forkPot] > kMaxPosf)
			{
				motor[fork] = 0;// Arm is fully raised. Stop the motor
			}
		...
		else if(vexRT[Btn6U]==0 && vexRT[Btn6D]==0) // No buttons are pushed.
		{
			...
			else if (bButtonsHaveBeenPressed)
			{
				...
				if(timeChange>=SampleTime)
				{
					...
					float dInput = (input - lastInput);
					/*Compute PID Output*/
					float output = kpf * error  - kdf * dInput;
					if(output > outMax) output = outMax;
					else if(output < outMin) output = outMin;
					motor[fork]= motor[fork]= output;
					lastInput = input;
					lastTime = now;
				}
			}
