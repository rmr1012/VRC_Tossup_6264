...
#pragma config(Motor,  port3,           backRight,     tmotorServoContinuousRotation, openLoop, reversed, encoderPort, I2C_1)
...
#pragma config(Motor,  port8,           backLeft,      tmotorServoContinuousRotation, openLoop, encoderPort, I2C_2)
...
//	joystick=128;
	...
	while(!(eyeM|eyeR|eyeL))//eyehits line)		//turn left
	{
	...
	{
		motor[backLeft]=motor[frontLeft]=79;
	...
	while(!eyeL&!eyeM&!eyeR)//eyehits line)		//drift
	{
		...
		if(nMotorEncoder[backLeft]>1000&nMotorEncoder[backLeft]<2000)
		{
...
//	while(!(SensorValue[GOL]|SensorValue[GOR])){}
	...
	motor[armmotL]=motor[armmotR]=-128;
	while(SensorValue[armPotentiometerL]>2280&SensorValue[armPotentiometerR]>2280)
	{
