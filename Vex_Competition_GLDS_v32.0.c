...
//	forward(128,350);
	...
	while(SensorValue[forkPot]>450+fpc){
		motor[backLeft]=motor[backRight]=50;
...
/*		sprintf(ddhang, " %2.0f%c ", dhang);
		...
		hangpos=SensorValue[armPotentiometerL];
		wait1Msec(100);
		dhang = hangpos - SensorValue[armPotentiometerL];
	}
	SensorValue[solLock]=true;
	joystickf=0;
	wait10Msec(300);
...
//	backward(80,300);
	...
	kMaxPos=2400;
	joystick=128;
	while(SensorValue[forkPot]>500+fpc);
	motor[fork]=0;
...
void GoRH2()
{
	...
	SensorValue[solBig]=0;
	turn(128,-135);
	while(!(SensorValue[GOR]|SensorValue[GOL]));
	openup=true;
	kMaxPosf=2500+fpc;
	joystickf=0;
	joystick=-128;
...
void GoBH2()
{
	...
	joystickf=128;
	backward(60,300);
	while(SensorValue[forkPot]<2700+fpc);
	kMotorSpeedUp   = 128;
	joystickf=-128;
	//	backward(60,400);
	...
	kMaxPosf=2500+fpc;
	joystickf=128;
	while(SensorValue[forkPot]<2800+fpc);
	wait1Msec(300);
	SensorValue[solBig]=true;
	wait1Msec(300);
	SensorValue[solBig]=false;
