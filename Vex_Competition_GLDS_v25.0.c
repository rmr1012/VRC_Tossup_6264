...
task pMonitor()
{
	...
	while(1)
	{
		bBig=SensorValue[solBig];
		bDump=SensorValue[solDump];
		bFork=SensorValue[fork1];
		bBreak=SensorValue[solBreak];
		bLock=SensorValue[solLock];
		bShift=SensorValue[solShift];
		...
		wait1Msec(10);
		if(bBig!=SensorValue[solBig])//constracted or extended
			cBig++;
		if(bDump!=SensorValue[solDump])//constracted or extended
			cDump++;
		if(bLock!=SensorValue[solLock])//constracted or extended
			cLock++;
		if(bShift!=SensorValue[solShift])//constracted or extended
			cShift++;
...
void turn(int power,float angle)
{
	...
	int error = 10;
	float const kt=0.15;
	float const kit=0.05;
	float const kdt=0.05;
	...
	if(decidegrees>=0)
	{
		...
		SensorValue[gyro]=0;
		while(abs(SensorValue[gyro]) < (float)decidegrees*0.7 )
		{
	...
	else
	{
		...
		SensorValue[gyro]=0;
		wait1Msec(10);
		while(SensorValue[gyro] > (float)decidegrees*0.7 )
		{
			wait1Msec(10);
			motor[frontRight] = -power;
			motor[backRight] = -power;
