...
task raiseOrLowerArmL()
{
	...
	float ITermL,ITermR;
	float ki=0.015,kd=0.8,kp=2;
	float static outMax=128,outMin=-128;
	int nPositionToHold;
	...
	while(true)
	{
		...
		if (joystick <= -90)// button is puchsed indicating ARM should be lowered
		{
			...
			nPositionToHold = ((SensorValue[armPotentiometerL]+SensorValue[armPotentiometerR])/2)-15;
		...
		else// No buttons are pushed.
		{
			...
			else if (bButtonsHaveBeenPressed)
			{
				...
				{
					static float inputL = SensorValue[armPotentiometerL];
					static float errorL = nPositionToHold - inputL;
					if(errorL>0&errorL-20>0)errorL=errorL-20;
					else if(errorL<0&errorL+20<0)errorL=errorL+20;
					else errorL=0;
					ITermL+= (ki * errorL);
					if(ITermL > outMax) ITermL= outMax;
					else if(ITermL < outMin) ITermL= outMin;
					float dInputL = (inputL - lastInputL);

					// Writes the current value of int 'x' to the debug stream.

					/*Compute PID Output*/
					float outputL = kp * errorL + ITermL- kd * dInputL;
					...
					writeDebugStream("		%d",SensorValue[armPotentiometerL]);
					writeDebugStream("		%d",motor[armmotL]);
